    ...
    class SeismicOverlayPatches
    {
        ...
        public static class SimDebugView_OnPrefabInit_Patch
        {
            ...
            private static Color GetColor(SimDebugView instance, int cell)
            {
                ...
                Color color3 = Tuning.Colors.seismicOverlayActive;

                float treshold = 0.5f;
                float value = SeismicGrid.activity[cell];
                if (value < treshold)
                    return Color.Lerp(color1, color2, value / treshold);
                else
                    return Color.Lerp(color2, color3, (value - treshold) / treshold);
            }
            private static Color GetEpicenterColors(SimDebugView instance, int cell)
            {
                if (WorldEventManager.Instance.currentEvent.affectedCells.TryGetValue(cell, out float val))
