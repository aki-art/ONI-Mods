...
namespace WorldTraitsPlus
{
    class FXPatches
    {
        internal static SpawnFXHashes megaMeteorFX = Tuning.FXHashes.MegaMeteorImpact;
        ...
        public static class Game_InitializeFXSpawners_Patch
        {
            public static void Prefix(ref SpawnPoolData[] ___fxSpawnData, Dictionary<int, Action<Vector3, float>> ___fxSpawner)
            {
                ...
                var megaMeteorImpact = CreateNew(Tuning.FXHashes.MegaMeteorImpact, meteorFX, 3f);
                var meteorImpactSteam = CreateNew(Tuning.FXHashes.MeteorImpactSteam, meteorFX, 1f, "contact_steam_kanim");

                spawnData.Add(megaMeteorImpact);
                spawnData.Add(meteorImpactSteam);
            ...
            {
                return new SpawnPoolData(
                    hash,
                    original.initialCount,
                    Color.white,
                    prefab,
                    original.initialAnim,
                    original.spawnOffset,
                    original.spawnRandomOffset,
                    original.rotationConfig,
