...
namespace Backwalls
{
    public class BackwallRenderer2 : MonoBehaviour
    {
        [SerializeField]
        ...
        private static bool MatchesDef(GameObject go, BackwallVariant variant)
        {
            return go != null && go.GetComponent<Backwall>()?.variant == variant;
        }
        ...
        public virtual Bits GetConnectionBits(int x, int y, int query_layer)
        {
            ...
            if (y > 0)
            {
                ...
                if (x > 0 && MatchesDef(Grid.Objects[cell - 1, query_layer], variant))
                {
                    bits |= Bits.DownLeft;
                }
                if (MatchesDef(Grid.Objects[cell, query_layer], variant))
                {
                    bits |= Bits.Down;
                }
                if (x < Grid.WidthInCells - 1 && MatchesDef(Grid.Objects[cell + 1, query_layer], variant))
                {
                    bits |= Bits.DownRight;
                }
            }
            ...
            if (x > 0 && MatchesDef(Grid.Objects[num2 - 1, query_layer], variant))
            {
                bits |= Bits.Left;
            }
            if (x < Grid.WidthInCells - 1 && MatchesDef(Grid.Objects[num2 + 1, query_layer], variant))
            {
                bits |= Bits.Right;
            }
            if (y < Grid.HeightInCells - 1)
            {
                ...
                if (x > 0 && MatchesDef(Grid.Objects[num3 - 1, query_layer], variant))
                {
                    bits |= Bits.UpLeft;
                }
                if (MatchesDef(Grid.Objects[num3, query_layer], variant))
                {
                    bits |= Bits.Up;
                }
                if (x < Grid.WidthInCells + 1 && MatchesDef(Grid.Objects[num3 + 1, query_layer], variant))
                {
                    bits |= Bits.UpRight;
                }
            }
        ...
        private class RenderInfo
        {
            ...
            public RenderInfo(BackwallRenderer2 backwallRenderer, int renderLayer, BackwallVariant variant)
            {
            ...
            internal void Rebuild(BackwallRenderer2 renderer, int chunkX, int chunkY, List<Vector3> vertices, List<Vector2> uvs, List<int> indices, List<Color> colors)
            {
