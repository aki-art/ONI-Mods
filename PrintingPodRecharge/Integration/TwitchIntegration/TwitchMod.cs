...
namespace PrintingPodRecharge.Integration.TwitchIntegration
{
	public class TwitchMod
	{
		public static void OnDbInit()
		{
			...
			var (leakEvent, leakGroup) = EventGroup.DefaultSingleEventGroup(PrintingPodLeakCommand.ID, Consts.EventWeight.Common, STRINGS.TWITCH.PRINTING_POD_LEAK.NAME);
			...
			var (helpfulEvent, helpfulGroup) = EventGroup.DefaultSingleEventGroup(HelpfulPrintsCommand.ID, Consts.EventWeight.Common, STRINGS.TWITCH.HELPFUL_PRINTS.NAME);
			helpfulEvent.AddListener(HelpfulPrintsCommand.Run);
			helpfulEvent.Danger = Danger.None;
			deckInst.AddGroup(helpfulGroup);

			var (uselessEvent, uselessEventGroup) = EventGroup.DefaultSingleEventGroup(UselessPrintsCommand.ID, Consts.EventWeight.Common, STRINGS.TWITCH.USELESS_PRINTS.NAME);
